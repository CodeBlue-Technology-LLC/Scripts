# ----- About: ----
    # Forked from https://github.com/BackupNerd/Backup-Scripts/tree/master/Billing%20Usage
    # Original author: Eric Harless, Head Backup Nerd - N-able
    # Added ability to create Additions if not already present and syncs GB usage minus specified fair use amount.
...
[CmdletBinding()]
    Param (
        ...
        [Parameter(Mandatory=$True)][String]$PartnerName = "Code Blue Technology LLC",                                                 ## Cove Partner Name (e.g. 'Code Blue Technology LLC')
        ...
        [Parameter(Mandatory=$False)][int]$PhysServerGBFU = 500,                                        ## Selected Size Fair Use
        [Parameter(Mandatory=$False)][int]$VirtServerGBFU = 500,                                        ## Selected Size Fair Use
        [Parameter(Mandatory=$False)][int]$WorkstationGBFU = 100,                                       ## Selected Size Fair Use
        ...
        [Parameter(Mandatory=$True)][String]$CWMAgreementName,                                            ## ConnectWise Manage Agreement Name (e.g. 'IT Services Agreement')
...
#region ----- Environment, Variables, Names and Paths ----
    ...
    $Cove2PSAProductMapping = @{
        ## UPDATE THESE VALUES to match the Product Identifiers in YOUR ConnectWise Manage instance
        ## Cove SKU Name  >>>   PSA Product/ SKU Name / Identifier (Product must be created in ConnectWise Manage)

        "PhysServerQty"     = "COVE-BACKUP-SERVER"          ## CHANGE to your CWM product identifier for Physical Servers
        "VirtServerQty"     = "COVE-BACKUP-VIRTUAL"         ## CHANGE to your CWM product identifier for Virtual Servers
        "WorkstationQty"    = "COVE-BACKUP-WORKSTATION"     ## CHANGE to your CWM product identifier for Workstations
        "DocumentsQty"      = "COVE-BACKUP-DOCUMENTS"       ## CHANGE to your CWM product identifier for Documents
        "M365UserQty"       = "COVE-BACKUP-365"             ## CHANGE to your CWM product identifier for M365 Users
        "ContinuityQty"     = "COVE-BACKUP-CONTINUITY"      ## CHANGE to your CWM product identifier for Continuity
        "BilledGBQty"       = "COVE-STORAGE-GB"             ## CHANGE to your CWM product identifier for Billed GB
...
#region ----- Backup.Management JSON Calls ----
    ...
    Function GetMVReport {
        ...
        $Script:MVReport = $Script:MVReportxls | select-object  Disti_Id,Disti,Disti_Legal,Disti_Ref,
                                                                ...
                                                                @{n='PhysicalServer';e='Null'},@{n='VirtualServer';e='Null'},@{n='Workstation';e='Null'},@{n='Documents';e='Null'},GBFU,BilledStorageGB
   ...
   Function Join-Reports {
    ...
    $Script:MVPlus | foreach-object { if ($_.PhysicalServer -gt 0) {
        $_.GBFU = $PhysServerGBFU 
        $_.BilledStorageGB = ($_.UsedStorageGB - $PhysServerGBFU)
        if ($_.BilledStorageGB -lt 0) {$_.BilledStorageGB = 0}
        }
    }
    $Script:MVPlus | foreach-object { if ($_.VirtualServer -gt 0) {
        $_.GBFU = $VirtServerGBFU
        $_.BilledStorageGB = ($_.UsedStorageGB - $VirtServerGBFU)
        if ($_.BilledStorageGB -lt 0) {$_.BilledStorageGB = 0}
        }
    }
    $Script:MVPlus | foreach-object { if ($_.Workstation -gt 0) {
        $_.GBFU = $WorkstationGBFU
        $_.BilledStorageGB = ($_.UsedStorageGB - $WorkstationGBFU)
        if ($_.BilledStorageGB -lt 0) {$_.BilledStorageGB = 0}
        }
    }
    ...
    $Script:MVPlus | Where-object {$_.CustomerState -eq "InProduction"} | where-object {$_.CustomerName -notlike '*Recycle Bin'} | 
                    ...
                    CustomerState,CreationDate,ProductionDate,DeviceDeletionDate,Now_TimeStamp,SelectedSizeGB,UsedStorageGB,GBF,BilledStorageGB |
    ...
    $Script:MVPlus | Where-object {$_.CustomerState -eq "InTrial"} | where-object {$_.CustomerName -notlike '*Recycle Bin'} | 
                    ...
                    OsType,CurrentMonthMvSKU,DeviceId,DeviceName,ComputerName,
                    Now_Product,Now_Physicality,PhysicalServer,VirtualServer,Workstation,Documents,RecoveryTesting,O365Users,
                    CustomerState,CreationDate,ProductionDate,DeviceDeletionDate,Now_TimeStamp,SelectedSizeGB,UsedStorageGB,GBFU,BilledStorageGB |
...
#region ----- ConnectWise Manage Body / Functions  ----
    if ($SendtoCWM) {
        ...
        Function UpdateCWMQty {
            $effectivedate = "$(Get-Date $period -format yyyy-MM-01)T00:00:00Z"
            $Additions = Get-CWMAgreementAddition -AgreementID $Agreement.id -all | 
                Where-Object {$null -eq $_.cancelledDate -or ([DateTime]::Parse($_.cancelledDate) -gt [DateTime]::Parse($start))}
            if ($debugCWM) {
            ...
            $physserver = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.PhysServerQty}
            $virtserver = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.VirtServerQty}
            $workstation = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.WorkstationQty}
            $documents = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.DocumentsQty}
            $m365user = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.M365UserQty}
            $continuity = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.ContinuityQty}
            $billedgb = $additions | where-object {$_.product.identifier -eq $Cove2PSAProductMapping.BilledGBQty}
            #physical server
            if ($physserver) {
                $Update = @{
                    AgreementID = $Agreement.id    
                    AdditionID = $physserver.id     
                    Operation = 'replace'     
                    Path = 'quantity'     
                } 
                Update-CWMAgreementAddition @Update -Value $CDPusage.PhysServerQty | out-null
                $update = $null
            }
            elseif ($CDPusage.PhysServerQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.PhysServerQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.PhysServerQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            }
            #virtual server
            if ($virtserver) {
                $Update = @{
                    AgreementID = $Agreement.id    
                    AdditionID = $virtserver.id     
                    Operation = 'replace'     
                    Path = 'quantity'     
                } 
                Update-CWMAgreementAddition @Update -Value $CDPusage.VirtServerQty | out-null
                $update = $null
            }
            elseif ($CDPusage.VirtServerQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.VirtServerQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.VirtServerQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            ...
            elseif ($CDPusage.WorkstationQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.WorkstationQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.WorkstationQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            ...
            elseif ($CDPusage.DocumentsQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.DocumentsQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.DocumentsQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            ...
            if ($continuity) {
                $Update = @{
                    AgreementID = $Agreement.id    
                    AdditionID = $continuity.id     
                    Operation = 'replace'     
                    Path = 'quantity'     
                } 
                Update-CWMAgreementAddition @Update -Value $CDPusage.ContinuityQty | out-null
                $update = $null
            }
            elseif ($CDPusage.ContinuityQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.ContinuityQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.ContinuityQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            ...
            if ($m365user) {
                $Update = @{
                    AgreementID = $Agreement.id    
                    AdditionID = $m365user.id     
                    Operation = 'replace'     
                    Path = 'quantity'     
                } 
                Update-CWMAgreementAddition @Update -Value $CDPusage.M365UserQty | out-null
                $update = $null
            }
            elseif ($CDPusage.M365UserQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.M365UserQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.M365UserQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            ...
            if ($billedgb) {
                $Update = @{
                    AgreementID = $Agreement.id    
                    AdditionID = $billedgb.id     
                    Operation = 'replace'     
                    Path = 'quantity'     
                } 
                Update-CWMAgreementAddition @Update -Value $CDPusage.BilledGBQty | out-null
                $update = $null
            }
            elseif ($CDPusage.BilledGBQty -gt 0){
                $Create = @{
                    AgreementID = $Agreement.id 
                    product = @{identifier = $Cove2PSAProductMapping.BilledGBQty}
                    billCustomer = 'Billable' 
                    quantity = $CDPusage.BilledGBQty
                    effectivedate = $effectivedate
                }
                New-CWMAgreementAddition @Create
                $create = $null
            ...
            $Additions = Get-CWMAgreementAddition -AgreementID $Agreement.id -all | Where-Object {$null -eq $_.cancelledDate -or ([DateTime]::Parse($_.cancelledDate) -gt [DateTime]::Parse($Start))}
            if ($debugCWM) {
        ...
        Function LookupCWMAgreements {
            ...
            $Agreements = Get-CWMAgreement -condition "name=`"$CWMAgreementName`" and cancelledflag=false" -all
        ...
        $Usage = $MVplus |
                ...
                PhysicalServer,VirtualServer,Workstation,Documents,RecoveryTesting,O365Users,BilledStorageGB | Sort-Object endcustomer -Unique 
        ...
        foreach ($customer in $Usage) {
            ...
            $CDPusage += New-Object -TypeName PSObject -Property @{
                ...
                BilledGBQty     = IF ($null -ne (($mvplus | Where-Object {($_.endcustomer -eq $customer.endcustomer) -and ($_.BilledStorageGB -ge 1)} | measure-object BilledStorageGB -sum).sum))
                    {($mvplus | Where-Object {($_.endcustomer -eq $customer.endcustomer) -and ($_.BilledStorageGB -ge 1)} | measure-object BilledStorageGB -sum).sum}else{0} ;
            }    
            ...
            Write-Output "  BilledGBQty      : $($CDPusage.BilledGBQty)"
            LookupCWMCompany
        }
    }else{ 
        Write-Output "  SendTo-ConnectWise Manage switch disabled" 
