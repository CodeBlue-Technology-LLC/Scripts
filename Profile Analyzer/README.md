...
## Overview
Three PowerShell scripts for identifying stale user profiles across different profile management platforms. All cross-reference Active Directory using .NET DirectoryServices — no ActiveDirectory module required.
...
Targets **User Profile Disk** shares containing `UVHD-*.vhdx` files named by SID (e.g., RDS with UPD). Uses VHDX `LastWriteTime` as the activity indicator — login activity is sufficient to update this timestamp. Resolves SIDs to AD accounts automatically.

### 3. Analyze-StaleFSLogix-NoModule.ps1
Targets **FSLogix profile container** shares. Folders follow the `SID_Username` naming convention, each containing a `Profile_Username.VHDX` inside. Username is parsed directly from the folder name — no SID translation needed for display. VHDX `LastWriteTime` is used as the activity indicator.
...
### Analyze-StaleFSLogix-NoModule.ps1

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `ProfilePath` | String | Yes | Path to FSLogix share containing SID_Username folders |
| `DaysInactive` | Integer | Yes | Days since last VHDX write to flag as stale (1-7300) |
| `ExportPath` | String | No | CSV export path (default: current directory with timestamp) |
...
### CSV Export
All scripts export a CSV with per-entry details. FSLogix and UPD scripts include SID and resolved display name. Profile folder script includes file count.

### Summary Statistics
- Total files/folders/disks scanned
- Stale count
...
All scripts use .NET `DirectoryServices` with no-argument `DirectoryEntry` binding — this uses the machine's own domain context and avoids UPN suffix vs. AD domain name mismatches (e.g., `domain.cc` UPN vs `domain.local` AD domain).

The FSLogix and UPD scripts resolve `SID → NTAccount → sAMAccountName` via LDAP. The profile folder script searches directly by folder name as `sAMAccountName`.
...
**No FSLogix folders found**
Confirm folders follow the `S-1-5-21-*_Username` naming convention. Any folder not matching that pattern is skipped automatically. A warning is logged for any matching folder that contains no VHDX inside.

**AD lookup errors in results**
Script is domain-joined and AD is reachable, but individual SID or username resolution failed. Do not treat as orphaned without manual verification.
...
Expected on large shares — the script must enumerate every file recursively. Run during off-hours or test on a subset first. The UPD and FSLogix scripts are significantly faster since they only read VHDX file metadata.
